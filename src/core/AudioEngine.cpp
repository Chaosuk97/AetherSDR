...
// ── Morse code table ────────────────────────────────────────────────────────
// Each entry is a null-terminated string of '.' (dit) and '-' (dah).
static const char* morseTable(QChar ch)
{
    static const struct { char c; const char* pattern; } table[] = {
        {'A', ".-"},    {'B', "-..."},  {'C', "-.-."},  {'D', "-.."},
        {'E', "."},     {'F', "..-."},  {'G', "--."},   {'H', "...."},
        {'I', ".."},    {'J', ".---"},  {'K', "-.-"},   {'L', ".-.."},
        {'M', "--"},    {'N', "-."},    {'O', "---"},   {'P', ".--."},
    ...
    for (int i = 0; table[i].c; ++i)
        if (table[i].c == upper) return table[i].pattern;
    return nullptr;
}

void AudioEngine::playSidetone(QChar ch, int wpm, int pitchHz)
{
    if (!m_sidetoneEnabled || !m_audioDevice || !m_audioDevice->isOpen())
        return;

    // Space = inter-word gap (7 dits) of silence
    if (ch == ' ' || ch == QChar(0x7f)) {
        int ditMs = 1200 / qBound(5, wpm, 100);
        int silenceMs = 7 * ditMs;
        int silenceSamples = (DEFAULT_SAMPLE_RATE * silenceMs) / 1000;
        QByteArray silence(silenceSamples * 2 * sizeof(int16_t), 0);
        if (m_resampleTo48k)
            m_audioDevice->write(resampleStereo(silence));
        else
            m_audioDevice->write(silence);
        return;
    }

    const char* pattern = morseTable(ch);
    if (!pattern) return;

    int ditMs = 1200 / qBound(5, wpm, 100);
    int ditSamples = (DEFAULT_SAMPLE_RATE * ditMs) / 1000;
    int gapSamples = ditSamples;  // intra-character gap = 1 dit
    constexpr int RAMP_SAMPLES = 120; // 5ms at 24kHz — cosine taper
    constexpr double TWO_PI = 6.283185307179586;
    double phaseInc = TWO_PI * pitchHz / DEFAULT_SAMPLE_RATE;

    // Build the full character's audio
    QByteArray charAudio;
    for (int i = 0; pattern[i]; ++i) {
        int elemSamples = (pattern[i] == '-') ? ditSamples * 3 : ditSamples;

        // Generate tone with cosine ramp envelope
        QByteArray tone(elemSamples * 2 * sizeof(int16_t), 0);
        auto* samples = reinterpret_cast<int16_t*>(tone.data());
        int ramp = qMin(RAMP_SAMPLES, elemSamples / 2);

        for (int s = 0; s < elemSamples; ++s) {
            double env = 1.0;
            if (s < ramp)
                env = 0.5 * (1.0 - cos(TWO_PI * 0.5 * s / ramp));
            else if (s >= elemSamples - ramp)
                env = 0.5 * (1.0 - cos(TWO_PI * 0.5 * (elemSamples - 1 - s) / ramp));

            double sample = env * 0.25 * sin(m_sidetonePhase);
            m_sidetonePhase += phaseInc;
            int16_t val = static_cast<int16_t>(sample * 32767.0);
            samples[s * 2]     = val;  // L
            samples[s * 2 + 1] = val;  // R
        }
        charAudio.append(tone);

        // Intra-character gap (silence between elements, not after last)
        if (pattern[i + 1]) {
            QByteArray gap(gapSamples * 2 * sizeof(int16_t), 0);
            charAudio.append(gap);
        }
    }

    // Keep phase wrapped to avoid precision loss
    m_sidetonePhase = fmod(m_sidetonePhase, TWO_PI);

    // Write to audio output
    if (m_resampleTo48k)
        m_audioDevice->write(resampleStereo(charAudio));
    else
        m_audioDevice->write(charAudio);
}
